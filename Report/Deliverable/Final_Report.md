...
The same physical cluster was used, but one hardware discrepancy was identified. The paper describes the `xl170` nodes as having two 10-core CPUs, while the provisioned nodes had one Intel Xeon E5-2640 v4 with 10 physical cores and 20 logical CPUs. The remaining listed hardware characteristics matched the evaluation environment. The Linux-Homa validation uses the HomaModule Linux `v6.17.8` kernel, while eTran uses the custom `6.6.0-eTran+` kernel. This difference is required by the two stack implementations being compared, rather than being an experimental substitution. The official eTran and HomaModule sources were used, and the benchmark parameters were taken from the metric definitions and runbooks rather than replaced with a separate workload or dataset.
...
| Metric                                 | Our measurement: eTran-Homa / Linux-Homa | Reported: eTran / Linux-Homa |
| -------------------------------------- | ---------------------------------------: | ---------------------------: |
